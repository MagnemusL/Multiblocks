...
public class MasterMenu extends AbstractContainerMenu {
    public final MasterBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public MasterMenu(int containerID, Inventory inv, FriendlyByteBuf extraData) {
        this(containerID, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public MasterMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.MASTER_MENU.get(), containerId);
        checkContainerSize(inv, 6);
        blockEntity = ((MasterBlockEntity) entity);
    ...
    private void addPlayerHotbar(Inventory playerInventory) {
        for (int i = 0; i < 9; ++i) {
            this.addSlot(new Slot(playerInventory, i, 7 + i * 18, 182));
        }
    }
}
